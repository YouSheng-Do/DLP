...
class Test_model(VAE_Model):
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        # TODO
        label_list = torch.unbind(label, dim=0)
        
        for i in range(1, self.val_vi_len):
            if i == 1:
                img_feature = self.frame_transformation(img[i-1])
            else:
                img_feature = self.frame_transformation(generate_img)
            label_feature = self.label_transformation(label_list[i])

            z = torch.from_numpy(np.random.normal(0, 1, [1, self.args.N_dim, self.args.frame_H, self.args.frame_W]).astype(np.float32)).to(self.args.device)
            # print(z.shape)
            decoded_feature = self.Decoder_Fusion(img_feature, label_feature, z)
