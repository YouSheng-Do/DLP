...
#mask_b: iteration decoding initial mask, where mask_b is true means mask
    def inpainting(self,image,mask_b,i): #MakGIT inference
        ...
        self.model.eval()
        with torch.no_grad():
            z_indices = self.model.encode_to_z(image).view(image.size(0), -1) #z_indices: masked tokens (b,16*16)
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='./transformer_checkpoints/cosine/epoch_10.pth', help='load ckpt')
